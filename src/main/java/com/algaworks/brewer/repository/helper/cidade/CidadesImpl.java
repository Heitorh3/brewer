...
public class CidadesImpl implements CidadesQueries {
	...
	private void adicionarFiltro(CidadeFilter filtro, Criteria criteria) {
		if (filtro != null) {
			...
			if (!StringUtils.isEmpty(filtro.getEstado())){
				criteria.add(Restrictions.like("estado", filtro.getEstado()));
			}
		}
	}
	...
	@Transactional
	@Override
	public Cidade buscarCidadeComEstados(Long codigo) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cidade.class);
		criteria.createAlias("estado", "e", JoinType.RIGHT_OUTER_JOIN);
